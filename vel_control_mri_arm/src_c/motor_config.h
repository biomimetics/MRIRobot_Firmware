...
typedef struct { /* __MOTOR_CONFIG_H */
    ...
    // LEGACY (rad/s) -- USM.lf bounds against the shared
    // MOTOR_MAX_SPEED_COUNTS_PER_SEC instead. Commented out with the rest of the
    // rad/s chain in common.h; USM_DAC.lf (imported by no Main) was its only
    // remaining reader.
    // float max_speed;
    ...
    // LEGACY -- this field was the per-encoder rad/s->duty reconciliation the
    // counts-domain switch exists to bypass (see counts_domain_io_plan.md).
    // USM.lf divides by the single shared PWM_COUNTS_PER_SEC_MAX and never read
    // this. Commented out along with PWM_RAD_PER_SEC_MAX in common.h.
